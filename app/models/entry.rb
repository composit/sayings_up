class Entry
  ...
  def as_json( options = {} )
    super( options.merge( only: [:_id, :content], include: { comments: { only: [:_id, :content], methods: [:exchange_id, :entry, :entry_user_id, :child_exchange_data] } }, methods: [:exchange_id, :user_username] ) )
  end
