...
describe ExchangesController do
  ...
  context 'POST' do
    ...
    let( :initial_values_hash ) { stub.as_null_object }
    let( :exchange ) { mock_model( Exchange, initial_values: initial_values_hash ).as_null_object }
    ...
    it 'assigns the current user' do
      initial_values_hash.should_receive( :[]= ).with :user_id, current_user.id
    end
